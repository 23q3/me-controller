...
配方(输入、产出、工序地址)由**样板**统一承载,持久化在 Lua 侧 `recipes.db`;目标只经 `recipeId` 引用样板,自身只保留库存策略(各产物目标库存、优先级、调度参数)。改样板即改所有引用它的目标;被目标引用的样板不可删除。旧 targets.db 里内嵌配方的目标会在 Lua 控制器下次启动时自动迁移:升格出样板并挂接,数据不丢。

**样板管理**页(样板 → 新增/编辑样板)是结构化行编辑,物品 ID 输入框带中文名/英文名/ID 自动补全(数据来自物品资产索引):
...
- **主产物**的"目标库存"驱动生产;**副产物**默认 0——只入账、不驱动生产(AE2 式副产物语义)。副产物目标设为正数则按共产物维持,批次数取各产物需求的最大值。
...
所有请求路径(自动维持决策、样板下单、手动请求缺料)都把一批原料的全部条目合并为**一次** `requestFiltered(address, filter1, filter2, ...)` 变参调用——Create 的 Stock Ticker 会把全部物品汇入同一个 `PackageOrder` 一次广播,发出的包裹携带同一订单号,理包机(Repackager)才能把它们整理成一个完整包裹。逐物品多次调用会生成多张订单,需要"同批同包"的产线(理包机 + 装配工序)会因此卡死——这是红线,勿改回。`request` 命令支持 `items: [{item, count}...]` 多物品形态(旧单物品 `item/count` 仍兼容);Stock Ticker 回包只有总数,短缺(库存竞态)时按下单量全额入账并记 WARN 事件,等承诺 TTL 自愈。
...
│   └── client/          # main、state(极简 store)、ws(重连)、api、render(全量重渲染)、target-editor、recipe-editor、dom
